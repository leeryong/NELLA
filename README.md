...
🤖  계획을 세웠습니다. 시작할까요?
      학습데이터 500쌍 생성 → 품질 검증 → Qwen2.5-1.5B 파인튜닝 → 평가

👤  진행해 주세요

🤖  ✅ 완료했습니다. 만들어진 모델과 대화해 보시겠어요?
```
...
| 섹션 | 내용 |
| --- | --- |
| [🆕 최신 소식](#news) | Windows 설치 파일 공개, TAW 에이전트 |
| [🎬 무엇을 할 수 있나요](#what) | 시연 영상과 화면으로 보는 NELLA |
| [🗂️ 10단계로 진행됩니다](#pipeline) | 문서 업로드부터 대화 테스트까지 |
| [🚀 주요 기능](#features) | 자동화되는 항목 정리 |
| [✅ 시작 전 준비물](#prepare) | API 키·GPU 등 필요한 것 |
| [🛠️ 설치 방법](#install) | Windows 설치 파일 / Docker |
| [📞 문의](#contact) | 연락처 |
| [👨‍💻 개발자 그룹](#team) | KISTI BLUESKY 팀 |
| [📚 활용 공개 소스](#oss) | 사용한 오픈소스 |
...
```
📄 문서 → 📝 학습데이터 → 🔍 검증 → 🤖 모델선택 → 🧪 사전검증
                                                      ↓
     💬 대화 테스트 ← 📚 RAG DB ← 📊 평가 ← 📈 결과확인 ← ⚙️ 훈련
```
