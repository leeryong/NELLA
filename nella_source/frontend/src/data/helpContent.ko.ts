...
export const helpContent_ko: Record<string, HelpEntry> = {
  documents: {
    pageTitle: "문서 업로드",
  ...
  ragDb: {
    pageTitle: "RAG DB 관리",
    summary: "업로드된 문서를 골라 검색 가능한 벡터DB(RAG DB)를 만듭니다. 여러 개의 RAG DB를 이름별로 만들어 두고, 다음 단계 '대화 테스트'에서 필요한 것을 골라 붙일 수 있습니다.",
    sections: [
      {
        title: "RAG DB란",
        items: [
          "문서 텍스트를 청크(chunk) 단위로 나눠 <b>BGE-M3</b> 임베딩으로 벡터화하여 <b>ChromaDB</b>에 저장한 것",
          "대화 시 사용자의 질문과 의미가 가까운 청크를 뽑아 컨텍스트로 넣어주는 <b>Retrieval-Augmented Generation</b> 기법",
          "모델의 지식 밖 정보(사내 매뉴얼·최신 문서 등)를 근거 기반으로 답하게 합니다",
        ],
      },
      {
        title: "생성 방법",
        items: [
          "우측 상단 <b>+ 새 RAG DB</b> 버튼으로 이름·설명·포함할 문서를 선택하고 생성",
          "생성 즉시 선택된 문서들이 임베딩되어 벡터DB에 인덱싱됩니다",
          "여러 개의 RAG DB를 목적별(예: '기술문서', '법령', '사내규정')로 나눠 관리할 수 있습니다",
        ],
      },
      {
        title: "관리 작업",
        items: [
          "<b>문서 편집</b> — 기존 RAG DB에 문서를 추가·제외 (제외 시 해당 문서 청크만 삭제)",
          "<b>재인덱스</b> — 문서 내용이 바뀌었거나 임베딩을 새로 만들고 싶을 때 전체 재구축",
          "<b>삭제</b> — RAG DB 자체를 삭제하면 Chroma 컬렉션도 함께 제거",
        ],
      },
      {
        title: "대화 테스트와 연결",
        items: [
          "'대화 테스트' 페이지의 <b>RAG 사용</b> 스위치를 켜면 RAG DB 드롭다운이 나타납니다",
          "이 페이지에서 만든 RAG DB 중 하나를 골라 대화에 사용하세요",
          "여러 개의 RAG DB로 답변 품질을 비교하며 실험할 수 있습니다",
        ],
      },
    ],
  },
